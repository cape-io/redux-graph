...
test('entityDelReducer', (t) => {
  ...
  const st1 = putRefs(state, { subject: agent, predicate: 'friends', object: creator })
  // console.log(state.Person.user0.rangeIncludes)
  const st2 = entityDelReducer(st1, entityDel(list).payload)
  t.deepEqual(st2.Person.user0.rangeIncludes.creator, {})
  t.deepEqual(st2.CollectionList, {})
  // Check mainEntity too.
  // console.log('agent1', agent)
  const st3 = entityDelReducer(st2, entityDel(agent).payload)
  console.log(st3.Person.user0.rangeIncludes.friends)
  t.deepEqual(st3.Person.user0.rangeIncludes.friends, {})
  // console.log(st2)
  t.end()
})
